...
function paginate(){
    ...
    .done(function(response){
        ...
        for(var i = 0; i < response.data.length; i++){
            ...
            var msg_collection = JSON.parse(escaper[0].innerHTML);
            table_html += `<tr>
                <td>${msg_collection.timestamp}</td>
                <td>${msg_collection.from}</td>
                <td>${msg_collection.to}</td>
                <td>${msg_collection.message}</td>
            </tr>`;
        };
        table_html += '</table>';
        $('#coll_docs').append(table_html);
